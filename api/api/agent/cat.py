...
import os
from api.agent.model_use import model_use
import random

# 正确加载 .env 文件
load_dotenv(dotenv_path=Path('.') / 'api' / 'agent' / '.env')
# print(Path('.') / '.env')
# 从环境变量中获取 bot_id 和 token
bot_id = os.getenv("cat_bot_id")
api_token = os.getenv("api_token")
# print(f"bot_id: {bot_id}, api_token: {api_token}")
...
if __name__ == "__main__":
    user_input = "你好，猫咪！"
    response = get_ai_reply(user_input)
    print(response)  # 输出 AI 的回复
    print(random_cat_phrase())  # 输出随机猫咪语录